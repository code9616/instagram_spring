...
import com.instagram.app.web.validation.auth.AuthValidaion;
...
public class AuthController {
	...
	public String getSigninSubmit(String username, String password, HttpServletRequest request) throws UnsupportedEncodingException {
		AuthValidaion authValidaion = new AuthValidaion();
		Map<Boolean, String> usernameIsNull = authValidaion.isNull("username", username);
		Map<Boolean, String> passwordIsNull = authValidaion.isNull("password", password);
		...
		if(user != null){
			...
			session.setAttribute("profileImg", user);
		}else {
			//로그인 실패 메세지 전달
